...
\begin{theorem}[adapted from\cite{aljawi2023eigenvalues}]
  ...
  for $c \in (0, 1)$.
\end{theorem}
\begin{proof} %TODO: rewrite to solve issues with divergence
Consider the ODE
...
\index{operator!finite difference}\index{operator!Jacobi}\index{matrix!Jacobi}
...
operator}\index{operator!monodromy}, which intuitively inscribes `what has
