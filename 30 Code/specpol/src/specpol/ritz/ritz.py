...
import numpy as np
from pyfilon import filon_fun_iexp, filon_fun_sin, filon_fun_cos

from specpol.common import generate_matrix
from specpol.algebra.operators import Operator
...
def ritz_bounded_L2(
    ...
    def entry_func(i, j):
        ...
        return (
            1
            / np.sqrt(b)
            * filon_fun_iexp(integrand(i), 0, b, -2 * j * np.pi / b, quad_mesh_size)
        )
...
def ptb_ritz(
    func: Callable, lda: float, matrix_size: int, quad_mesh_size: int, dbm: bool
) -> np.array:
    """
    Approximate the spectrum of a perturbed multiplication operator M_f on L^2(0, 1)
    ...
    quad_mesh_size: int
        ...
        Must be an odd integer greater than 1.
    dbm: bool
        Whether to add a dissipative barrier to show that the eigenvalue created
        by the perturbation is indeed a real eigenvalue.
    ...
    def integrand(n):
        if n == 0:  # need to separate out this case to avoid a divide by 0
            return lambda x: func(x) + phi_squared + 1j * (abs(n) <= 25 and dbm == True)
        return lambda x: (
            func(x) * np.exp(2j * n * np.pi * x)
            + -1j * (-1 + np.exp(2j * n * np.pi)) / (2 * np.pi * n) * phi_squared
            + 1j * (abs(n) <= 25 and dbm == True) * np.exp(2j * n * np.pi * x)
        )
